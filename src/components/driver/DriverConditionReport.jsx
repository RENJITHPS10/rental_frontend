...
const DriverConditionReport = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      formData.append('type', reportType); // 'before' or 'after'
  ...
  // Determine report type based on booking status and reports
  ...
  // Updated conditions:
  // - "Before" report allowed when status is 'pickup-confirmed' and no "before" report exists
  // - "After" report allowed when status is 'delivered' and "before" report exists
  const canReportBefore = booking?.status === 'pickup-confirmed' && !hasBeforeReport;
  const canReportAfter = booking?.status === 'completed' && hasBeforeReport && !hasAfterReport;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full mx-auto bg-gray-800/50 backdrop-blur-md border border-gray-700/50 rounded-3xl shadow-2xl p-8 relative z-10">
        <h2 className="text-4xl font-extrabold text-center bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 mb-6 animate-text-glow">
          {reportType === 'before' ? 'Report Condition Before Pickup' : 'Report Condition After Delivery'}
        </h2>
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              {reportType === 'before' ? 'Condition Before Pickup' : 'Condition After Delivery'}
            </label>
            <textarea
              name="condition"
              placeholder={`Describe vehicle condition ${reportType === 'before' ? 'before pickup' : 'after delivery'}`}
